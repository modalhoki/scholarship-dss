...
class GradeEdit extends Grade
{
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'lgd' first before field var 'x_lgd'
        $val = $CurrentForm->hasValue("lgd") ? $CurrentForm->getValue("lgd") : $CurrentForm->getValue("x_lgd");
        if (!$this->lgd->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->lgd->Visible = false; // Disable update for API request
            } else {
                $this->lgd->setFormValue($val);
            }
        }

        // Check field name 'interview' first before field var 'x_interview'
        $val = $CurrentForm->hasValue("interview") ? $CurrentForm->getValue("interview") : $CurrentForm->getValue("x_interview");
        if (!$this->interview->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->interview->Visible = false; // Disable update for API request
            } else {
                $this->interview->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->lgd->CurrentValue = $this->lgd->FormValue;
        $this->interview->CurrentValue = $this->interview->FormValue;
    }
    ...
    protected function validateForm()
    {
        ...
        if ($this->lgd->Required) {
            if ($this->lgd->FormValue == "") {
                $this->lgd->addErrorMessage(str_replace("%s", $this->lgd->caption(), $this->lgd->RequiredErrorMessage));
            }
        }
        if ($this->interview->Required) {
            if ($this->interview->FormValue == "") {
                $this->interview->addErrorMessage(str_replace("%s", $this->interview->caption(), $this->interview->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if ($this->awardee->CurrentValue != "") { // Check field with unique index
            $filterChk = "(`awardee` = " . AdjustSql($this->awardee->CurrentValue, $this->Dbid) . ")";
        ...
        if (!$rsold) {
            ...
            // lgd
            $this->lgd->setDbValueDef($rsnew, $this->lgd->CurrentValue, null, $this->lgd->ReadOnly);

            // interview
            $this->interview->setDbValueDef($rsnew, $this->interview->CurrentValue, null, $this->interview->ReadOnly);
